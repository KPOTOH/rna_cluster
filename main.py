...
# ===== 2 Там выбрасывается около 2/3 данных сделаем так же. Однако это бесполезно, ведь данные совсем не нормальные
        # и выбрасываются просто шумовые значения от несовершенства эксперимента
...
# ===== 3 Тут, исходя из смысла, надо оставить примерно верхнюю половину значений
...
# ===== 4 Опять-таки, по графику из тетрадки видно, что оставляют меньшую часть клеток,
#         видимо, ту, где экспрессия в целой клетке была достаточно значимой, не шумовой.
        # Ведь если с клетки мало значимых ридов, то что-то пошло не так, и ее лучше устранить
# little_part = 100
